...
class BaseKemenagPonses:
    ...
    async def encrypt_id_emis(id: int):
        async with ClientSession(
            # request_class=ProxyConnector
        ) as session:
            async with session.get('http://192.168.29.154:3000/v1/encryptIDEmis',
                                   params={
                                       'id': id
                                   },
    ...
    async def _get_by_city(self, **kwargs):
        ...
        while(True):
            ...
            for institution in results:
                a = await self._get_all_detail_institution(institution=institution, **kwargs)
                if(a) :paths.append(
                    a
                )
    ...
    async def _get_all_detail_institution(self, **kwargs):
        try:
            a1 = await self._get_detail_institution((institution_id := (institution := kwargs.pop('institution'))['institution_id']))
            b1 = await  self._get_table_institution(institution_id, academic_year_id=(tahun_ajaran := kwargs.pop('tahun_ajaran')).value)
            data: dict = {
                    "link": (link := 'https://emis.kemenag.go.id/pontren/statistik/pontren'),
                    "domain": (link_split := link.split('/'))[2],
                    "tag": link_split[2:],
                    "crawling_time": Datetime.now(),
                    "crawling_time_epoch": int(time()),
                    f'detail_{kwargs.pop("institution_id").name.lower()}': reduce(lambda a, b: dict(a or {}, **(b or {})), [
                        a1,b1]
                    ) | institution,
                    **kwargs,
